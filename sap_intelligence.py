...
SIMILARITY_TOKEN_PATTERN = re.compile(r"[A-Za-z0-9_/.-]+")
...
def ocr_is_available():
    ...
    if find_spec("pytesseract") is None:
        return False
    return bool(get_tesseract_executable())
...
def get_neural_nlp_mode():
    return str(os.getenv("SAP_NEURAL_NLP_MODE", "lazy")).strip().lower()
...
def sentence_model_is_warm():
    return load_sentence_model.cache_info().currsize > 0
...
def similarity_tokens(text):
    tokens = []
    for token in SIMILARITY_TOKEN_PATTERN.findall(normalize_whitespace(text).lower()):
        token = token.strip("._-/")
        if not token or token in SIMILARITY_STOPWORDS:
            continue
        if len(token) < 3 and not token.isdigit():
            continue
        tokens.append(token)
    return safe_unique(tokens, limit=80)


@lru_cache(maxsize=1)
def load_reference_lexical_items():
    items = []
    for item in load_reference_items():
        lexical_item = dict(item)
        lexical_item["tokens"] = set(similarity_tokens(item["text"]))
        items.append(lexical_item)
    return items
...
def lexical_similarity_score(query_tokens, item_tokens):
    if not query_tokens or not item_tokens:
        return 0.0
    overlap = len(query_tokens & item_tokens)
    if overlap == 0:
        return 0.0
    return overlap / sqrt(len(query_tokens) * len(item_tokens))


def rank_similarity_entries(scored, top_k=5):
    ...
    for entry in scored:
        ...
        selected.append({key: value for key, value in entry.items() if key != "rank_score"})
        counts[bucket] += 1
...
def lexical_matches(text, top_k=5):
    query = normalize_whitespace(text)
    if not query:
        return []

    query_tokens = set(similarity_tokens(query))
    if not query_tokens:
        return []

    scored = []
    for item in load_reference_lexical_items():
        score = lexical_similarity_score(query_tokens, item["tokens"])
        if score < 0.18:
            continue
        type_bonus = 0.0
        if item["type"] == "system":
    ...
    return rank_similarity_entries(scored, top_k=top_k)
...
def semantic_matches(text, top_k=5):
    ...
    # Keep first-response latency low by avoiding heavyweight model loads on the
    # interactive path unless the model was already warmed or the user opted in.
    if get_neural_nlp_mode() != "always" and not sentence_model_is_warm():
        return lexical_matches(query, top_k=top_k)
    if not neural_nlp_is_available():
        return lexical_matches(query, top_k=top_k)
